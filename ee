...
local function Hide(Interface, JustHide: boolean?, Notify: boolean?, Bind: string?)
	...
	-- Simply disable the interface instead of animating transparency
...
function Starlight:CreateWindow(WindowSettings)
	...
	-- Load saved UI keybind
	if not isStudio and isfile then
	    local keybindPath = Starlight.FileSystem.Folder .. "/ui_keybind.txt"
	    if isfile(keybindPath) then
	        pcall(function()
	            local savedKey = readfile(keybindPath)
	            if savedKey and savedKey ~= "" and savedKey ~= "No Bind" then
	                Starlight.WindowKeybind = savedKey
	            end
	        end)
	    end
	end
	...
	do
		...
		task.spawn(function()
			if WindowSettings.LoadingEnabled then
				...
				title.playerName.Visible = false
	...
	function Starlight.Window:CreateHomeTab(TabSettings)
		...
		-- Add after the Server section in the Home Tab
		Tab.Instances.Page.Holder.Right.UIKeybind = Tab.Instances.Page.Holder.Right.Server:Clone()
		Tab.Instances.Page.Holder.Right.UIKeybind.Parent = Tab.Instances.Page.Holder.Right
		Tab.Instances.Page.Holder.Right.UIKeybind.Name = "UIKeybind"
		Tab.Instances.Page.Holder.Right.UIKeybind.Header.Text = "UI Keybind"
		Tab.Instances.Page.Holder.Right.UIKeybind.Subheader.Text = "Press to change the keybind\nto show/hide the UI"

		-- Clear the frame content
		for _, child in pairs(Tab.Instances.Page.Holder.Right.UIKeybind.Frame:GetChildren()) do
		...
		keybindButton.BackgroundColor3 = Starlight.CurrentTheme.Backgrounds.Dark
		keybindButton.BorderSizePixel = 0
		keybindButton.Font = Enum.Font.GothamBold
		keybindButton.Text = Starlight.WindowKeybind
		keybindButton.TextColor3 = Starlight.CurrentTheme.Foregrounds.Light
		keybindButton.TextSize = 18
		keybindButton.Parent = Tab.Instances.Page.Holder.Right.UIKeybind.Frame
		...
		ThemeMethods.bindTheme(keybindButton, "BackgroundColor3", "Backgrounds.Dark")
		ThemeMethods.bindTheme(keybindButton, "TextColor3", "Foregrounds.Light")
		ThemeMethods.bindTheme(stroke, "Color", "Foregrounds.Dark")
			...
			connection = UserInputService.InputBegan:Connect(function(input, processed)
				...
				if input.UserInputType == Enum.UserInputType.Keyboard then
					if input.KeyCode ~= Enum.KeyCode.Unknown then
						...
						Starlight.WindowKeybind = keyName
						keybindButton.Text = keyName

						-- Save to file if possible
						if not isStudio and writefile then
							pcall(function()
								if not isfolder(Starlight.FileSystem.Folder) then
									makefolder(Starlight.FileSystem.Folder)
								end
								writefile(Starlight.FileSystem.Folder .. "/ui_keybind.txt", keyName)
							end)
						end
						...
						Starlight:Notification({
							...
							Icon = 6026568227,
							Content = string.format("UI keybind changed to %s", keyName),
						})
	...
	function Starlight.Window:CreateTabSection(Name: string, Visible)
		...
		function TabSection:CreateCustomTab(TabSettings, TabIndex)
			...
			if Starlight.Window.CurrentTab == nil then
			    Starlight.Window._FirstTab = Tab
			end
		...
		function TabSection:CreateTab(TabSettings, TabIndex)
							...
							local function close()
								...
								Tween(NestedElement.Instances[2], { Size = UDim2.fromOffset(0, 0) }, function()
									if NestedElement and NestedElement.Instances ~= nil then
										...
										if acrylicFlag and AcrylicObject and AcrylicObject.Model then
			...
			function Tab:BuildThemeGroupbox(Column, Style, ButtonsCentered)
				...
				-- UI Hide Keybind
				local uiKeybindLabel = instance:CreateLabel({
				    Name = "UI Toggle Keybind",
				    Icon = 6031071057,
				    Tooltip = "Change the keybind to show/hide the UI"
				}, "ui_keybind_label")
				    ...
				    OnChangedCallback = function(newKey)
				        ...
				        if not isStudio and writefile then
				            pcall(function()
				                ...
				                writefile(Starlight.FileSystem.Folder .. "/ui_keybind.txt", newKey)
				            end)
				        end
				        ...
				        Starlight:Notification({
				            Title = "Keybind Changed",
				            Icon = 6026568227,
				            Content = string.format("UI hide keybind changed to %s", newKey),
				            Duration = 2,
				        })
				    end,
				}, "ui_keybind")
	...
	-- Ensure MainTab is selected after loading
	task.spawn(function()
	    -- Wait for all tabs to be created
	    task.wait(0.5)
	
	    -- Find MainTab specifically
	    local mainTab = nil
	    for _, tabSection in pairs(Starlight.Window.TabSections) do
	        if tabSection.Tabs["MainTab"] then
	            mainTab = tabSection.Tabs["MainTab"]
	            break
	        end
	    end
	
	    -- Activate MainTab if found
	    if mainTab and mainTab.Instances and mainTab.Instances.Button then
	        -- Deactivate all tabs first
	        for _, tabSection in pairs(Starlight.Window.TabSections) do
	            for _, tab in pairs(tabSection.Tabs) do
	                tab.Active = false
	                if tab.Instances and tab.Instances.Button then
	                    Tween(tab.Instances.Button, { BackgroundTransparency = 1 })
	                    Tween(tab.Instances.Button.Icon, { ImageColor3 = Starlight.CurrentTheme.Foregrounds.Medium })
	                    Tween(tab.Instances.Button.Header, { TextColor3 = Starlight.CurrentTheme.Foregrounds.Medium })
	                    tab.Instances.Button.Icon.AccentBrighter.Enabled = false
	                    tab.Instances.Button.Header.AccentBrighter.Enabled = false
	                end
	            end
	        end
	
	        -- Activate MainTab
	        Tween(mainTab.Instances.Button, { BackgroundTransparency = 0.5 })
	        Tween(mainTab.Instances.Button.Icon, { ImageColor3 = Starlight.CurrentTheme.Foregrounds.Light })
	        Tween(mainTab.Instances.Button.Header, { TextColor3 = Starlight.CurrentTheme.Foregrounds.Light })
	        mainTab.Instances.Button.Icon.AccentBrighter.Enabled = true
	        mainTab.Instances.Button.Header.AccentBrighter.Enabled = true
	        mainTab.Active = true
	        Starlight.Window.CurrentTab = mainTab
	        tabs.UIPageLayout:JumpTo(mainTab.Instances.Page)
	    end
	end)
